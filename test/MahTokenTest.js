...
contract('MahToken', function(accounts) {
  var tokenInstance;

  it('initializes the contract with the correct values', function() {
    return MahToken.deployed().then(function(instance) {
      tokenInstance = instance;
      return tokenInstance.name();
    }).then(function(name) {
      assert.equal(name, 'MahToken', 'token contains the correct name');
      return tokenInstance.symbol();
    }).then(function(symbol) {
      assert.equal(symbol, 'MAH', 'token contains the correct symbol');
      return tokenInstance.standard();
    }).then(function(standard) {
      assert.equal(standard, 'MahToken v0.0.1', 'token contains the correct standard');
    });
  });
  ...
  it('can transfer tokens from one account to another', function() {
    return MahToken.deployed().then(function(instance) {
        ...
        var expectedSenderBalance = 1999750;
